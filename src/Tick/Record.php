...
abstract class Record extends Entity {
	 ...
	 *
	 * @param boolean $insert If we use the output for insert unique field can be empty
	 *
	 * @return array
	...
	protected function hydrate($insert = true) {
		...
		foreach ($this->listPropertyNames() as $property) {
			...
			$data[$this->propertyAlias($property)] = array('type' => $this->propertyType($property), 'value' => $this->$property);
		}
		return $data;
	}
	...
	public function createCriteria($property, $condition, $value) {
		return array('property' => $property, 'condition' => $condition, 'value' => $value);
	}

	/**
	 * Get list of object matching criterias
	 *
	 * @param array $criterias Key => Value pairs to search for
	 *
	...
	protected function getAdvanced(array $criterias) {
		...
		foreach ($criterias as $criteria) {
			...
			$result[] = array('property' => $this->propertyAlias($criteria['property']), 'condition' => $criteria['condition'], 'value' => $criteria['value']);
		}
		return $result;
	}
