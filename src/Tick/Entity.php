...
abstract class Entity extends Type {
	 ...
	 *
	 * @param boolean $modified Set the object as modified
	 *
	 * @return boolean
	 */
		...
		if (!key_exists($class, self::$_propertyMap)) {
			...
			foreach (explode("\n", $this->getClassComment()) as $line) {
				if (preg_match($propertyBasePattern, $line, $mathes)) {
					...
					$size = (int)$mathes[2];
					$name = $mathes[3];
					$property = array("type" => $type, "default" => null);
					...
					if (preg_match_all($optionsPattern, $rest, $mathes, PREG_PATTERN_ORDER)) {
						...
						foreach ($mathes["options"] as $option) {
							if (!empty($option)) {
								if (in_array($option, array("null", "unique"))) {
			...
			foreach ($properties as $name => $props) {
				$fields[$props["field"]] = array_merge($props, array("property" => $name));
			}
